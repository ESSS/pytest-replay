...
class ReplayWriter(object):
    ...
    def append_test_to_script(self, report):
        ...
        fn = os.path.join(self.dir, self.BASE_SCRIPT_NAME + worker_suffix + '.' + self.ext)
        if not os.path.isfile(fn):
            with open(fn, 'w') as f:
                f.write(self.template.preamble)

        if report.nodeid in self.written_nodeids:
            return
        with open(fn, 'a') as f:
            f.write(self.template.test.format(nodeid=report.nodeid))
            self.written_nodeids.add(report.nodeid)
